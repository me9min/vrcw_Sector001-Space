...
public class DayCycle : UdonSharpBehaviour
{
    ...
    private void Update()
    {
        ...
        sun.transform.eulerAngles = new Vector3(hour,sun.transform.rotation.y,sun.transform.rotation.z);

        //RenderSettings.skybox.SetFloat("_Rotation", hour);

        // deltaTime : 1fps = 1, 50fps = 0.02, 100fps = 0.01
        hour += Time.deltaTime*15f; // hour=1분/실제1초 == hour=0.25

        // 24시 넘을시 24시간 나눠서 나머지를 적용  예) hour가24.4면 0.4로, 49면 1로
        if (360f <= hour)
        {
            hour = hour%360f;
        }
